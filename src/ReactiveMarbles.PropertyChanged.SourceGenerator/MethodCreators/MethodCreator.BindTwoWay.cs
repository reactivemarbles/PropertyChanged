...
namespace ReactiveMarbles.PropertyChanged.SourceGenerator.MethodCreators
{
    ...
    {
        private static List<StatementSyntax> CreateTwoWayBindStatements(ExpressionArgument hostExpressionArgument, ExpressionArgument targetExpressionArgument, bool hasConverters, bool isExtension, List<ExpressionChain> hostExpressionChains, List<ExpressionChain> targetExpressionChains)
        {
            var statements = new List<StatementSyntax>();

            var hostInputType = hostExpressionArgument.InputType.ToDisplayString();
            var hostOutputType = hostExpressionArgument.OutputType.ToDisplayString();

            var targetInputType = targetExpressionArgument.InputType.ToDisplayString();
            ...
            var hostObservableChain = GetObservableChain(fromName, hostExpressionChains, Constants.WhenChangedEventName, Constants.WhenChangedEventHandler);
            ...
            var targetObservableChainInside = GetObservableChain(fromName, targetExpressionChains, Constants.WhenChangedEventName, Constants.WhenChangedEventHandler);
        ...
        private static MethodDeclarationSyntax CreateBindTwoWayMethod(string hostInputType, string hostOutputType, string targetInputType, string targetOutputType, bool isExtension, bool hasConverters, Accessibility accessibility, List<StatementSyntax> statements)
        {
            ...
            statements.Add(ThrowStatement(ObjectCreationExpression(Constants.InvalidOperationExceptionTypeName, new[] { Argument("\"No valid expression found.\"") }), 1));

            var body = Block(statements, 1);
            return MethodDeclaration(modifiers, Constants.SystemDisposableTypeName, Constants.BindTwoWayMethodName, parameterList, 0, body);
        }
    }
}
